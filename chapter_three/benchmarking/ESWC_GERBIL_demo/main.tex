...
\newcommand{{\overalldata}}{11\xspace}
%ACE2004
%AQUAINT
%MSNBC
%IITB
...
%Spotlight Corpus
%Microposts
\newcommand{{\numberOfadditionaldata}}{6\xspace}
...
\title{Evaluating Entity Annotators Using GERBIL}
...
GERBIL is an open-source and extensible framework that allows evaluating tools against (currently) \overallGERBILannotators different annotators on \overalldata sets different data sets within 6 different experiment types. 
...
\includegraphics[width=0.7\linewidth]{gerbiloverview.eps}
%\vspace{-5mm}
...
\label{fig:spiderfmeasure}
}~
\subfigure[Spider diagram of correlations between annotation results and data set features.]{
\includegraphics[width=0.48\textwidth]{correlations.pdf}
\label{fig:spiderfeature}
}
...
%\begin{table}[htb]
%\centering
%\captionof{table}{Implemented annotator systems. }
%\label{tab:annotator}
%\begin{tabulary}{\columnwidth}{LLC}
%\toprule
...
The selection and an explanation of the types of matching for given experiments will be part of the demo (see Figure~\ref{fig:screenshot}). %\todo[inline]{AN: Add ref to figure as requested above.}
...
%Table~\ref{tab:annotator} presents the provided systems and their supported experiment types.
...
Table~\ref{tab:corpus_stats} shows the \overalldata sets data sets available via GERBIL. 
...
This result in itself is of high practical significance as it means that by using GERBIL, developers can evaluate on (currently) \overalldata sets data sets using the same effort they needed for 1, which is a gain of more than 1100\%.
...
\includegraphics[width=0.48\textwidth]{user_study.eps}
\label{ref:comparedTime}
%\captionof{figure}{Example spider diagram of recent A2KB experiments with weak annotation matching derived from our online interface.}
}~
\subfigure[Runtime per document for the A2KB experiment type on the MSNBC data set.]{
\includegraphics[width=0.48\textwidth]{needed_times.eps}
\label{fig:runtime}
%\captionof{figure}{Spider diagram of results of figure \ref{fig:spiderfmeasure} and their correlation to data set features.}
}
