...
\title{Developing a Sustainable Platform for Entity Annotation Benchmarks}
...
An important novel feature of our interface is that it displays the correlation between the features of data sets and the performance of tools (see Figure~\ref{fig:spiderfeature}). By these means, we ensure that developers can easily gain an fine-grained overview of the performance of tools
...
(1) A user can upload a NIF-based corpus (see Section~\ref{sec:datastructures}) via our web-interface. 
...
\begin{figure}[tb]
    \centering
    \includegraphics[width=0.9\linewidth]{gerbiloverview.eps}
...
\end{figure}
Figure \ref{fig:architecture} shows the architecture of GERBIL with the data sets at the bottom, the annotators in the top and the user interface as well as user defined annotator and data set at the right.
