...
\graffito{
This chapter presents AGDISTIS, an novel approach towards graph-based, knowledge base-agnostic entity linking using Linked Data. 
AGDISTIS' methods and features have been published in~\cite{agdistisdemo, agdistis_ecai, agdistis_iswc}.
The author of this thesis implemented and co-evaluated AGDISTIS and its demo. 
...
Current \ac{NED} approaches suffer from two major drawbacks:
...
\item We evaluate our approach on nine \emph{well-known and diverse open-source datasets}\footnote{Further data, detailed experimental results and source code for this paper are publicly available on our project homepage \url{http://aksw.org/Projects/AGDISTIS}.} as well as against state-of-the-art named entity disambiguation frameworks.
...
%We formalize the task of \ac{NED} in Section~\ref{sec:ned}.
Section~\ref{sec:eval} evaluates AGDISTIS performance against state-of-the-art tools, nine diverse datasets, three knowledge bases and three languages. 
...
For our example, the graph depicted in Figure~\ref{fig:example} shows an excerpt of the input graph for the  \ac{HITS} disambiguation algorithm when relying on DBpedia as knowledge base. 
...
To evaluate the Chinese version of AGDISTIS, a Chinese benchmark is created in the context of \ac{QA}.
